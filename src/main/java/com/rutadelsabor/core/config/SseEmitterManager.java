...
public class SseEmitterManager {
    ...
    // CopyOnWriteArrayList es thread-safe para iteración concurrente sin bloquear
    ...
    public void publicar(String evento, Object datos) {
        List<SseEmitter> muertos = new ArrayList<>();
        for (SseEmitter emitter : emitters) {
            try {
                emitter.send(SseEmitter.event()
                        .name(evento)
                        .data(datos, MediaType.APPLICATION_JSON));
            } catch (IOException e) {
